...
@section('content')
        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <h5>Master Jadwal Praktek Laboratorium</h5>
                ...
                <div class="ibox-content">
                    ...
                    <div class="row m-b-sm m-t-sm">
                        <div class="col-md-4">
                            <form action="{{ route('backend.master.jadwallablist') }}" method="GET">
                                <div class="input-group"><input autocomplete="off" type="text" id="cari" name="cari" placeholder="Pencarian berdasarkan nama lab" class="input-sm form-control" value="{{ $cari }}"> <span class="input-group-btn">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            @foreach ($lab as $data => $value)
                                <tr>
                                    ...
                                    <td>{{ $value->kuota }}</td>
                                    <td>{{ $value->hari }}</td>
                                    <td>{{ $value->jadwal_start }}</td>
                                    <td>{{ $value->jadwal_end }}</td>
                                    <td>
    ...
    <form id="deleteform" action="{{ route('backend.master.jadwallabdelete') }}" method="POST" style="display: none;">
        @csrf
...
@section('onpage-js')
    ...
    <script>
        ...
        function createModal() {
            ...
            $('#tambahModal  .modal-body').load('{{ route('backend.master.jadwallabcreateview') }}');
        ...
        function editModal(pid) {
            ...
            $('#tambahModal  .modal-body').html('<i class="fa fa-cog fa-spin fa-2x fa-fw"></i> Loading...');
            $('#tambahModal  .modal-body').load('{{ route('backend.master.jadwallabeditview') }}?pid=' + pid);
            $("#tambahModal").modal('show');
        }
        ...
        function deleteWarning(uid) {
            swal({
                ...
                text: "Anda yakin akan menghapus jadwal praktek lab ini?",
                type: "warning",
