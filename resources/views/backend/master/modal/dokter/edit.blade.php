<form action="{{ route('backend.master.dokteredit') }}" method="post" enctype="multipart/form-data">
    @csrf
    <input type="hidden" id="iddokter" name="iddokter" value="{{ $dokter->id }}">
    <input type="hidden" id="fotolama" name="fotolama" value="{{ $dokter->foto }}">
    <div class="form-group">
        <label>Foto lama</label>
        <img alt="image" class="img-rounded img-md form-control" src="{{ env('API_PATH').'/uploads/profile-dokter/'.$dokter->foto }}">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <input type="text" id="kodedokter" name="kodedokter" value="{{ $dokter->kodedokter }}" placeholder="minimal 3 karakter huruf, angka, dash dan underscore, tidak menerima spasi." class="form-control" required>
    </div>
    <div class="form-group">
